"""
    make_bet(api::AbstractAPI, header, order)
...
```
"""
function make_bet(api::AbstractAPI, header, order)
    response = HTTP.request("POST", api.api_url * "bet", header, JSON3.write(order))
